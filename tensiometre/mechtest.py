...
def move_to_constant_positions(ab2xz, outnames, dxs, dzs, durations, kp=0.9, moveback=False):
    ...
    with closing(DT3100('169.254.3.100')) as sensorA, closing(DT3100('169.254.4.100')) as sensorB, closing(MPC385()) as actuator:
        ...
        #setting up PID
        pids = []
        initialposition = [x0, z0] + actuator.um2step(np.array([X0, Z0]))
        for s in initialposition:
            pid = PID(kp, 0, 0)
            pid.setPoint = s
            pids.append(pid)
        try:
            for outname, dx, dz, duration in zip(outnames, dxs, dzs, durations):
                for s, pid in zip(initialposition + actuator.um2step(np.array([dx,dz])), pids):
                    pid.setPoint = s
...
def move_to_constant_position(outname, ab2xz, kp=0.9, dz =-100, dx=0, duration=None, moveback=False):
    """Moving the absolute position of the head by dz and dx and stay at that position using PID feedback. Need ab2xz calibration matrix. Duration is in seconds. If None specified, continues until stopped."""
    move_to_constant_positions(ab2xz, outnames=[outname], dxs=[dx], dzs=[dz], durations=[duration], kp=kp, moveback=moveback)
